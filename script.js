...
// batas bintang Mythic+
const MYTHIC_START   = 0;    // ⭐1
const HONOR_START    = 25;   // ⭐25
const GLORY_START    = 50;   // ⭐50
const IMMORTAL_START = 100;  // ⭐100
...
function rankToStar(rank, div, star){
  if(RANK_DIVISI.includes(rank)) return RANK_ORDER.indexOf(rank)*STAR_PER_DIV*5 + DIVISI.indexOf(div)*STAR_PER_DIV + star;
...
function tampilInvoice(start,end,price,title){
  let detail={}; RANK_ORDER.forEach(r=>detail[r]=0);
  let total=0;
...
function hitungAntarRank(){
  tampilInvoice(rankToStar(rankA.value,divA.value,+starA.value),
...
function hitungGendongRank(){
  tampilInvoice(rankToStar(rankGA.value,divGA.value,+starGA.value),
...
function estimasiNominal(){
  ...
  let cur = start, saldo=+nominal.value, used=0;
  while(true){
    let r;
...
// ======================
// AUTO HIDE DIVISI + LIMIT STAR
// ======================
function updateDivisi(rankElId, divElId, starElId, wrapperId){
...
// APPLY TO ALL MENUS
updateDivisi("rank1","div1","star1","wrapperDiv1");
updateDivisi("rankA","divA","starA","wrapperDivA");
updateDivisi("rankB","divB","starB","wrapperDivB");
updateDivisi("rankG1","divG1","starG1","wrapperDivG1");
updateDivisi("rankGA","divGA","starGA","wrapperDivGA");
updateDivisi("rankGB","divGB","starGB","wrapperDivGB");
updateDivisi("rankE","divE","starE","wrapperDivE");
